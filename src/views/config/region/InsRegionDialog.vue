<template>
  <el-dialog
    :title="title"
    :visible="insRegionDialogVisible"
    width="30%"
    :before-close="handleClose"
    @close="provinceClosed"
    append-to-body
  >
    ...
    <span slot="footer" class="dialog-footer">
      <el-button size="medium" type="info" @click="handleClose()"
        >取 消</el-button
      >
      <el-button size="medium" type="success" @click="saveRegion()"
        >确 定</el-button
      >
    </span>
  </el-dialog>
...
import { selRegion, saveRegion } from '@/api/config/region'
export default {
  ...
  methods: {
    ...
    saveRegion() {
      this.$refs['form'].validate(async (valid) => {
        ...
        const res = await saveRegion(this.saveOrUpdate, this.province)
        if (res.status === 200) {
