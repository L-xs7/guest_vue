    ...
    >
      ...
      <el-form-item label="重定向:">
        <el-select
          style="width: 216px"
          size="medium"
          v-model="menu.metas.redirect"
        >
          <el-option
            v-for="item in redirectSelectData"
...
import { menuSaveOrUpd,menuList,menuById } from '@/api/config/menu'
export default {
  name: 'InsMenuDialog',
  data() {
    return {
  ...
  methods: {
    ...
    insMenu() {
      this.$refs['menuRuleFormRef'].validate(async (valid) => {
        if (!valid) return
        const res = await menuSaveOrUpd(this.menu)
        if (res.status === 200) {
